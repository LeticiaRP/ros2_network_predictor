...
# Threshold Defaults for the 72-Scenario Matrix
...
class LatencyForecaster(Node):
    def __init__(self):
        ...
        self.declare_parameter('safety_threshold', -1.0) # Use -1 to trigger auto-mapping
        self.declare_parameter('failure_threshold', -1.0)
        self.declare_parameter('alpha', 0.6) # Increased alpha for higher sensitivity

        # 2. Extract Scenario and Dynamic Thresholds
        ...
        # Automatic Threshold Selection based on Scenario
...
def main(args=None):
    ...
    if hasattr(node, 'model'): rclpy.spin(node)
    rclpy.shutdown()
