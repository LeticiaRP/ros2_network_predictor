...
# CALIBRATION THRESHOLDS (in ns)
# tau: Baseline P95 (~0.25ms); F: Failure Threshold (~0.7ms)
TAU = 250000.0 
F_LIMIT = 700000.0
...
def generate_stress_data():
    ...
    latency[350:] += np.linspace(0, 600000.0, 50) # Drive into Failure Zone
...
def run_stress_test():
    ...
    for i in range(len(raw_data) - input_len):
        ...
        with torch.no_grad():
            p95 = model(tensor).numpy()[0, 0, 2] # Extract P95
    ...
    # Axis 2: Risk Index
    ax2 = ax1.twinx()
    ax2.plot(plot_x, risk_indices, 'r', linewidth=2, label='Risk Index (RI)')
    ax2.set_ylabel("Risk Index [0, 1]", color='r')
    ax2.tick_params(axis='y', labelcolor='r')
    ax2.set_ylim(-0.1, 1.1)

    # Labeling Events
    events = [(50, "Freq Drop"), (150, "Best Effort"), (250, "Sawtooth"), (350, "Death Spiral")]
    for x, txt in events:
        plt.axvline(x=x, color='gray', linestyle='--', alpha=0.5)
        ax1.text(x+2, 0.75, txt, fontweight='bold')

    plt.title("Stress Test: Latency vs. Risk Index Anticipation")
    fig.legend(loc='upper left', bbox_to_anchor=(0.15, 0.85))
    plt.grid(True, alpha=0.2)
    plt.savefig("stress_test_with_ri.png")
